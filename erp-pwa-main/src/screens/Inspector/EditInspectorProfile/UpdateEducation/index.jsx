...
const EducationForm = ({ defaultValues }) => {
  ...
  useEffect(() => {
    const initialExperiences =
      inspector?.user?.inspector_details?.education || [];
  ...
  return (
    <>
      ...
      {inspector_loading ? (
        ...
        <>
          <TopBar title={"Edit Education"} show_back />
          <div className="pb-16">
            <form onSubmit={handleSubmit(onSubmit)}>
              ...
              <div className="px-5 py-2">
                {experiences.map((experience, index) => (
                  <div key={index}>
                    <h2
                      className={`text-sm font-semibold grow ${index >= 1 && "mt-5"
                        }`}
                    >
                      {parseInt(index) + 1} Education
                    </h2>
                    ...
                    <Controller
                      ...
                      render={({ field }) => (
                        <input
                          {...field}
                          placeholder="Educcation names"
                          className={`w-full border ${errors?.experiences?.[index]
                              ? "border-red-500"
                              : "border-[#E2E8F0]"
